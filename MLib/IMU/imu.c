...
trans_xyz_f_t reference_v;
ref_t 	ref;

//xyz_f_t Gravity_Vec;  				//解算的重力向量
	
float Roll,Pitch,Yaw,Continuous_YAW;    				//姿态角
...
{		
	...
	float mag_norm_tmp;
	static trans_xyz_f_t mag_tmp;
	static float yaw_mag;
	float yaw_date1;
	
	mag_norm_tmp = 20 *(6.28f *half_T);	
	
	mag_norm_xyz = my_sqrt(IST8310.Mag_Val.x * IST8310.Mag_Val.x + IST8310.Mag_Val.y * IST8310.Mag_Val.y + IST8310.Mag_Val.z * IST8310.Mag_Val.z);
	
		if( mag_norm_xyz != 0)
	{
		mag_tmp.x += mag_norm_tmp *( (float)IST8310.Mag_Val.x /( mag_norm_xyz ) - mag_tmp.x);
		mag_tmp.y += mag_norm_tmp *( (float)IST8310.Mag_Val.y /( mag_norm_xyz ) - mag_tmp.y);	
		mag_tmp.z += mag_norm_tmp *( (float)IST8310.Mag_Val.z /( mag_norm_xyz ) - mag_tmp.z);	
	}
...
// 				//Yaw   = ( - fast_atan2(2*(ref_q[1]*ref_q[2] + ref_q[0]*ref_q[3]),ref_q[0]*ref_q[0] + ref_q[1]*ref_q[1] - ref_q[2]*ref_q[2] - ref_q[3]*ref_q[3]) )* 57.3;
	 yaw_date1= fast_atan2(2*(-ref_q[1]*ref_q[2] - ref_q[0]*ref_q[3]), 2*(ref_q[0]*ref_q[0] + ref_q[1]*ref_q[1]) - 1) *57.3f  ;// 
	*yaw=yaw_date1;
	//*yaw = yaw_mag;
	Continuous_YAW=YawEncoderProcess(&C_YAW, yaw_date1);
}
...
{ 
	...
	v->ecd_angle = v->raw_value + v->round_cnt * 360;	
	return v->ecd_angle;
}
