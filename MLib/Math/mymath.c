...
float ratio_to_k(float in_x,float in_y,float in_z,float in_old_x,float in_old_y,float in_old_z)
{ float k;
	in_x=ABS(in_x);
	in_y=ABS(in_y);
	in_z=ABS(in_z);
	in_old_x=ABS(in_old_x);
	in_old_y=ABS(in_old_y);
	in_old_z=ABS(in_old_z);
	
  if(in_x >=in_y)
 {
  if(in_x >=in_z) {k=in_old_x/in_x;}
	else {k=in_old_z/in_z;}
 }
 else //in_y >in_x
 {
  if(in_y >=in_z) {k=in_old_y/in_y;}
	else {k=in_old_z/in_z;} //in_z >=in_y
 }
 return k;
}
float To_get_min(float date1,float date2)
{
	float returndate;
	if(date1>date2) {returndate=date2;}
	else{returndate=date1;}
	return returndate;
}
float To_get_max(float date1,float date2)
{
	float returndate;
	if(date1>date2) {returndate=date1;}
	else{returndate=date2;}
	return returndate;
}
